...
(p <- pheatmap(
  ...
  border_color=F,
  breaks=seq(-0.35, 0.35, length.out=101)
))
...
for (de in de.dfs) {
  de.genes <- de %>% 
    filter(Log.P.Value > -log10(0.2) & abs(Log.Fold.Change) > 0.2) %>%
    dplyr::select(Gene)
  de.genes <- de.genes[,1]
...
for (de in de.dfs) {
  ...
  kegg <- gseKEGG(de.kegg.genes, organism="hsa", nPerm=1000, minGSSize=1, maxGSSize=200, pvalueCutoff=0.1, verbose=F)
...
```{r}
os.d.a <- which(keggs[[1]]$ID == "hsa04612")
p <- gseaplot(keggs[[1]], geneSetID=os.d.a, by="runningScore", title=keggs[[1]]$Description[os.d.a]) +
  basic_theme_big +
  labs(title="KEGG Pathway for Antigen Processing and Presentation in Subtype A")
p$layers[[1]]$aes_params = list(colour = "#049e78", size=0.75)
p
ggsave(filename="results/8_DE_pathway_analysis/antigen_gene_ranks_A.svg", plot=p, width=10, height=3)

os.d.b <- which(keggs[[2]]$ID == "hsa04612")
q <- gseaplot(keggs[[2]], geneSetID=os.d.b, by="runningScore", title=keggs[[2]]$Description[os.d.b]) +
  basic_theme_big +
  labs(title="KEGG Pathway for Antigen Processing and Presentation in Subtype B")
q$layers[[2]]$aes_params = list(colour = "#f85900", size=0.75)
q
ggsave(filename="results/8_DE_pathway_analysis/antigen_gene_ranks_B.svg", plot=q, width=10, height=3)
```
...
for (de.id in 1:length(de.dfs)) {
  ...
  maps <- c("hsa05010", "hsa04380", "hsa04062", "hsa04670", "hsa04140", "hsa04612")
  for (map in maps) {
