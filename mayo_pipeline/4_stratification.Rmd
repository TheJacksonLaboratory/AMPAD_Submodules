...
```{r load_data}
eigengenes <- readRDS("clean_data/3_iterative_WGCNA_cleaning/mayo_eigengenes.RDS")

module.names <- readRDS("clean_data/3_iterative_WGCNA_cleaning/mayo_submodule_labels.RDS")
module.names <- unlist(module.names)

sig.genes <- readRDS("clean_data/3_iterative_WGCNA_cleaning/mayo_submodule_sig_genes.RDS")

cleaned <- readRDS("clean_data/1_mayo_cleaning/mayo_cleaned.RDS")
...
Looking at the results, I decided to use Ward D2 since it has the best silhouette results. The methods predicted 3-4 clusters.
...
subtypes <- data.frame(
  ...
  Subtype=c("A", "B", "C")[as.numeric(nbc.res$Best.partition[rownames(module.vecs)])],
  stringsAsFactors=F
)
subtypes$Subtype[is.na(subtypes$Subtype)] = "Control"
subtypes$Subtype <- factor(subtypes$Subtype, levels=c("A", "B", "C", "Control"))
rownames(subtypes) <- subtypes$Patient

saveRDS(nbc.res, "clean_data/4_stratification/mayo_clustering.RDS")
saveRDS(subtypes, "clean_data/4_stratification/mayo_subtypes_map.RDS")
...
ggplot(data=plot.data) +
  ...
  guides(size=FALSE) +
  basic_theme_big
ggsave(filename="results/4_stratification/mayo_clustering.svg", width=8, height=7.5)
```
...
# APOE4 Status
trait <- factor(c("None", "Heterozygous", "Homozygous")[as.numeric(cleaned[rownames(subtypes),"Tissue.APOE4"])-3])
plot.data <- subtypes
plot.data$Trait <- trait
ggplot(data=na.omit(plot.data)) +
  geom_bar(aes(x=Subtype, fill=Trait), position=position_fill(), color="black") +
...
colnames(patient.subtypes) <- c("Patient", "Subtype", "A", "B", "C")
...
patient.subtypes$C <- as.numeric(as.character(patient.subtypes$C))

saveRDS(patient.subtypes, "clean_data/4_stratification/mayo_patient_subtypes.RDS")
```
...
mayo.ids <- read.csv("raw_data/AMP-AD_Mayo_WGS_covariates.csv", stringsAsFactors=F)

# Load EMMAX .tfam file and order data by expected patients
tfam <- read.table("raw_data/Mayo_allSNPjointMAF05_Clean__TRANSPOSED_EMMAX.tfam")
colnames(tfam) <- c("Family", "WGS.ID", "X1", "X2", "X3", "X4")

# Merge data into subtype map by RNA Seq IDs
emmax.pheno <- merge(subtypes, mayo.ids, by.x="Patient", by.y="RNAseq_TCX_SampleID") %>% 
  dplyr::select(Patient, Subtype, WGS.ID=WGS_Participant_ID) %>% 
...
emmax.pheno <- merge(emmax.pheno, patient.subtypes, by.x="Patient", by.y="Patient") %>%
  dplyr::select(Patient, Subtype=Subtype.x, WGS.ID, A, B, C) %>%
  dplyr::filter(Patient %in% subtypes$Patient) %>%
  unique

# Merge data into eigengenes
emmax.pheno <- merge(emmax.pheno, eigengenes, by.x="Patient", by.y="Patient") %>%
  dplyr::select(Patient, Subtype, WGS.ID, A, B, C, module.names) %>%
  dplyr::filter(Patient %in% subtypes$Patient) %>%
  unique
...
emmax.pheno <- merge(tfam, emmax.pheno, by.x="WGS.ID", by.y="WGS.ID", all.x=T) %>%
  dplyr::select(c("WGS.ID", "A", "B", "C", module.names))
...
emmax.pheno <- emmax.pheno %>%
  dplyr::select(c("A", "B", "C", module.names))
...
modules <- split(
  modules %>% 
    dplyr::filter(brainRegion=="TCX") %>%
    dplyr::select(Gene=GeneID, Module=Module), 
  modules$Module[modules$brainRegion=="TCX"]
)
...
module.eigens <- cbind(rownames(module.eigens), module.eigens)
colnames(module.eigens)[1] <- "Patient"

# Merge data into subtype map by RNA Seq IDs
emmax.pheno <- merge(module.eigens, mayo.ids, by.x="Patient", by.y="RNAseq_TCX_SampleID") %>% 
  dplyr::select(Patient, WGS.ID=WGS_Participant_ID, colnames(module.eigens)) %>% 
