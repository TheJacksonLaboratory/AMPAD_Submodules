...
cleaned <- cleaned[grep("FP", cleaned$Tissue.Diagnosis),]
...
modules <- split(
  modules %>% 
    dplyr::filter(brainRegion=="FP") %>%
    dplyr::select(Gene=GeneID, Module=Module), 
  modules$Module[modules$brainRegion=="FP"]
)
